...
let verifiedChannelLiveCache={time:0,result:null};
async function loadVerifiedChannelLive(){
  const now=Date.now();
  if(verifiedChannelLiveCache.result&&now-verifiedChannelLiveCache.time<20000){
    return verifiedChannelLiveCache.result;
  }

  const sources=[
    'https://img-api-proxy.magsipocarnie.workers.dev/live-streams',
  ...
  // verified stream disappear or rebuild the iframe. Never keep this beyond 90s.
  if(verifiedChannelLiveCache.result&&now-verifiedChannelLiveCache.time<90000){
    return verifiedChannelLiveCache.result;
  }
