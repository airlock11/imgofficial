...
async function load(){
 ...
 renderGames(live,upcoming,finals);
 renderPrevious(finals);
...
function gameCard(g){
 ...
 '<div class="team"><div class="team-badge">'+short(a)+'</div><div><div class="team-name">'+a+'</div><div class="status-sub">'+(isFinal?"Final score":"Away")+'</div></div></div>'+
 '<div class="game-center">'+center+'</div>'+
 '<div class="team right"><div><div class="team-name">'+h+'</div><div class="status-sub">'+(isFinal?"Final score":"Home")+'</div></div><div class="team-badge">'+short(h)+'</div></div>'+
 '</div>';
}
function renderGames(live,upcoming,finals){
 const map={live:live,upcoming:upcoming,results:finals};
 const target=qs("#game-slot");
 const paint=k=>{target.innerHTML=gameCard((map[k]||[])[0]);qsa(".tab").forEach(b=>b.classList.toggle("active",b.dataset.tab===k))};
 qsa(".tab").forEach(b=>b.onclick=()=>paint(b.dataset.tab));
 paint(live.length?"live":upcoming.length?"upcoming":"results");
}
function renderPrevious(finals){
 ...
 wrap.innerHTML=finals.slice(0,8).map(g=>'<article class="media-card"><div class="media-content"><div class="media-kicker">PBA · Final</div><div class="media-title">'+g.away+' '+g.awayScore+' — '+g.homeScore+' '+g.home+'</div><div class="media-meta">'+(g.displayTime||fmtDate(g.date))+'</div></div></article>').join("");
}
...
function rows(list){return (list||[]).map((x,i)=>'<tr><td>'+(i+1)+'</td><td>'+x.team+'</td><td>'+x.wins+'</td><td>'+x.losses+'</td></tr>').join("")}
function renderOfficial(o){
 o=o||{};
 qs("#group-a").innerHTML=rows(o.rankings&&o.rankings.groupA);
 qs("#group-b").innerHTML=rows(o.rankings&&o.rankings.groupB);
