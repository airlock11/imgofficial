...
'<div class="img-dashboard"><div class="img-stack"><section class="img-panel img-reveal" id="imgGamesPanel" hidden><div class="img-panel-head"><div><h2>Games</h2><p>Live, upcoming and recent results</p></div><a class="img-panel-link" href="/scores/?from=league&league='+encodeURIComponent(cfg.key)+'">View all games ›</a></div><div class="img-game-tabs" id="imgGameTabs"><button type="button" data-game-mode="live">Live</button><button type="button" data-game-mode="upcoming">Upcoming</button><button type="button" data-game-mode="results">Results</button></div><div class="img-game-strip" id="imgGames"></div></section>'+
...
'<section class="img-panel img-reveal" id="imgStandingsPanel"><div class="img-panel-head"><div><h2>Standings</h2><p>Current table from connected data</p></div></div><div class="img-standings" id="imgStandings"></div></section>'+
'<section class="img-panel img-reveal" id="imgNewsPanel"><div class="img-panel-head"><div><h2>Latest '+esc(cfg.name)+' news</h2><p>Current league coverage from IMG and official sources</p></div></div><div class="img-news-list" id="imgNews"></div></section></div></div></div>';
...
function updateGameMode(){if(!modes.length){q('#imgGamesPanel').hidden=true;return}const m=modes[modeIndex%modes.length],list=m.items;q('#imgGames').innerHTML=list.map(gameCard).join('');q('#imgGamesPanel').hidden=false;q('#imgGameTabs')?.querySelectorAll('[data-game-mode]').forEach(b=>{const key=b.dataset.gameMode;b.classList.toggle('active',(key==='live'&&m.label==='Live now')||(key==='upcoming'&&m.label==='Upcoming')||(key==='results'&&m.label==='Results'));b.disabled=!modes.some(x=>(key==='live'&&x.label==='Live now')||(key==='upcoming'&&x.label==='Upcoming')||(key==='results'&&x.label==='Results'))})}
q('#imgGameTabs')?.addEventListener('click',e=>{const b=e.target.closest('[data-game-mode]');if(!b||b.disabled)return;const key=b.dataset.gameMode,idx=modes.findIndex(x=>(key==='live'&&x.label==='Live now')||(key==='upcoming'&&x.label==='Upcoming')||(key==='results'&&x.label==='Results'));if(idx>=0){modeIndex=idx;updateGameMode()}});
...
async function archivedLocalFinals(){
 try{
   const [a,b,c,h]=await Promise.allSettled([
     ...
     fetch('/regional-web.json?v='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null),
   ...
   addLeague(a.value);addLeague(b.value);addLeague(c.value);
...
async function localLeague(){
 const [a,b,c]=await Promise.allSettled([
   fetch('/special-sports-data.json?v='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null),
   fetch('/extended-sports-data.json?v='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null),
   fetch('/regional-web.json?v='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null)
 ]);
 const A=a.status==='fulfilled'?a.value:null,B=b.status==='fulfilled'?b.value:null,C=c.status==='fulfilled'?c.value:null;
 const candidates=[C?.leagues?.[cfg.key],C?.leagues?.[slug],A?.leagues?.[cfg.key],A?.leagues?.[slug],B?.leagues?.[cfg.key],B?.leagues?.[slug]].filter(Boolean);
 if(!candidates.length)return null;
 const base={...candidates[candidates.length-1]};
 const games=[],seen=new Set();
 for(const item of candidates){
   for(const g of(Array.isArray(item?.games)?item.games:[])){
     const k=String(g?.eventId||[g?.date,g?.away,g?.home].join('|'));if(!k||seen.has(k))continue;seen.add(k);games.push(g)
   }
   if(!base.standings&&item?.standings)base.standings=item.standings;
   if(!base.sourceName&&item?.sourceName)base.sourceName=item.sourceName;
 }
 base.games=games;
 return base;
}
...
async function getEvents(){if(cfg.local){const l=await localLeague();lastSource=l?.sourceName||'IMG data';return(Array.isArray(l?.games)?l.games:[]).map(specialToEvent)}if(cfg.regional){
   const [liveRes,localRes]=await Promise.allSettled([
     fetch(worker+'/regional-scores?league='+encodeURIComponent(cfg.key),{cache:'no-store'}).then(r=>r.ok?r.json():null),
     localLeague()
   ]);
   const live=liveRes.status==='fulfilled'&&liveRes.value?liveRes.value:null;
   const local=localRes.status==='fulfilled'?localRes.value:null;
   const rows=[...(Array.isArray(live?.events)?live.events:[]),...(Array.isArray(local?.games)?local.games.map(specialToEvent):[])],seen=new Set();
   lastSource=[live?.source,local?.sourceName,local?.league].filter(Boolean).join(' + ')||'IMG regional sources';
   return rows.filter(e=>{const t=teamPair(e),k=String(e?.id||e?.eventId||[eventDate(e),t.away.name,t.home.name].join('|'));if(seen.has(k))return false;seen.add(k);return true});
 }if(cfg.score){const now=new Date(),a=new Date(now-60*86400000),b=new Date(now.getTime()+21*86400000),url=cfg.score+(cfg.score.includes('?')?'&':'?')+'dates='+dateKey(a)+'-'+dateKey(b);try{const r=await fetch(url,{cache:'no-store'});if(r.ok){const j=await r.json();setLeagueIdentityFromPayload(j);lastSource=j?.leagues?.[0]?.name||'Connected sports feed';return j.events||[]}}catch{}const r=await fetch(worker+'/scoreboard?league='+encodeURIComponent(cfg.key)+'&dates='+dateKey(a)+'-'+dateKey(b),{cache:'no-store'});if(!r.ok)throw 0;const j=await r.json();lastSource='IMG proxy';return j.events||[]}return[]}
...
function renderKpis(live,up,finals){const next=up[0],nextPair=next?teamPair(next):null,stamp=new Intl.DateTimeFormat(undefined,{hour:'numeric',minute:'2-digit'}).format(new Date());const data=[['Live now',live.length?live.length+' game'+(live.length>1?'s':''):'No live game'],['Next game',nextPair?nextPair.away.name+' vs '+nextPair.home.name:'Check back soon'],['Recent results',finals.length?finals.length+' available':'No recent result'],['Source',lastSource],['Last updated',stamp]];q('#imgKpis').innerHTML=data.map(x=>'<div class="img-kpi img-reveal"><small>'+esc(x[0])+'</small><strong>'+esc(x[1])+'</strong></div>').join('');observe()}
...
let leagueSourceCache=null;
async function leagueSourceMeta(){
 if(leagueSourceCache)return leagueSourceCache;
 try{
  const r=await fetch('/league-sources.json?v=20260923-4',{cache:'no-store'});
  if(r.ok){const j=await r.json();leagueSourceCache=j?.leagues?.[slug]||{};return leagueSourceCache}
 }catch{}
 leagueSourceCache={};return leagueSourceCache
}
function sourceFallback(host,title,url,copy){
 if(!host)return;
 host.innerHTML='<a class="img-source-fallback" href="'+esc(url||'/sports/')+'" target="'+(/^https?:/i.test(url||'')?'_blank':'_self')+'" rel="noopener noreferrer"><small>Verified source</small><strong>'+esc(title)+'</strong><span>'+esc(copy)+'</span></a>'
}
async function renderStandings(){const panel=q('#imgStandingsPanel'),host=q('#imgStandings');panel.hidden=false;try{if(cfg.local){const l=await localLeague(),st=l?.standings;if(Array.isArray(st)&&st.length){host.innerHTML=table(st);return}if(st&&typeof st==='object'){const g=Object.entries(st).filter(([,v])=>Array.isArray(v)&&v.length);if(g.length){host.innerHTML=g.map(([n,v])=>'<h4>'+esc(n.replace(/([A-Z])/g,' $1'))+'</h4>'+table(v)).join('');return}}}if(cfg.stand){const r=await fetch(cfg.stand,{cache:'no-store'});if(r.ok){const rows=standardEntries(await r.json());if(rows.length){host.innerHTML=table(rows.slice(0,40));return}}}const src=await leagueSourceMeta();sourceFallback(host,cfg.name+' standings',src.standings||src.rankings||src.stats||src.official,'Open the current official standings, rankings or statistics source while IMG continues syncing structured table data.')}catch{const src=await leagueSourceMeta();sourceFallback(host,cfg.name+' standings',src.standings||src.rankings||src.stats||src.official,'Current standings source')}}
async function renderNews(){const panel=q('#imgNewsPanel'),host=q('#imgNews');panel.hidden=false;try{const r=await fetch('/news-data.json?v='+Date.now(),{cache:'no-store'});if(!r.ok)throw 0;const j=await r.json(),rx=new RegExp(cfg.news,'i'),items=(j.items||[]).filter(x=>rx.test((x.title||'')+' '+(x.description||''))).slice(0,9);if(items.length){host.innerHTML=items.map(x=>'<a class="img-news-item" href="'+esc(x.link)+'" target="_blank" rel="noopener noreferrer">'+(x.image?'<img src="'+esc(x.image)+'" alt="" loading="lazy" referrerpolicy="no-referrer">':'<span></span>')+'<div class="img-news-copy"><strong>'+esc(x.title)+'</strong><span>'+esc(x.source||'Sports news')+' · '+esc(dt(x.published))+'</span></div></a>').join('');return}const src=await leagueSourceMeta();sourceFallback(host,'Latest '+cfg.name+' news',src.news||src.official,'Open the current official league news source while IMG waits for the next matching story in its feed.')}catch{const src=await leagueSourceMeta();sourceFallback(host,'Latest '+cfg.name+' news',src.news||src.official,'Current official league news')}}
...
async function renderReels(clips){const panel=q('#imgHighlightsPanel'),host=q('#imgReels');panel.hidden=false;if(!clips.length){const src=await leagueSourceMeta();host.innerHTML='<a class="img-highlight-fallback" href="'+esc(src.highlights||src.official||'/sports/')+'" target="_blank" rel="noopener noreferrer"><span class="img-highlight-fallback-play">▶</span><small>Official highlights</small><strong>'+esc(cfg.name)+'</strong><span>Open the verified league video source while IMG waits for a playable clip to enter the feed.</span></a>';observe();return}host.innerHTML=clips.map((c,i)=>'<article class="img-reel" data-reel="'+i+'">'+(c.thumb?'<img src="'+esc(c.thumb)+'" alt="" loading="lazy" referrerpolicy="no-referrer">':'')+'<div class="img-reel-gradient"></div><span class="img-reel-source">'+esc(c.label||'Highlight')+'</span><div class="img-reel-info"><strong>'+esc(c.title)+'</strong><span>Tap to play</span></div><button class="img-reel-play" type="button" aria-label="Play highlight"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></button></article>').join('');host.querySelectorAll('.img-reel').forEach((el,i)=>el.querySelector('button').onclick=()=>playReel(el,clips[i]));observe()}
